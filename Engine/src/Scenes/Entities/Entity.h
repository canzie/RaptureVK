...
class Entity {
  ...
  public:
    ...
    Entity(uint32_t handle, Scene *scene) : m_EntityHandle(static_cast<entt::entity>(handle)), m_Scene(scene) {}
    ...
    template <typename... T> std::tuple<T *...> tryGetComponents()
    {
        if (!isValid()) {
    ...
    template <typename T, typename Func> void patchComponent(Func &&func)
    {
  ...
  private:
    ...
    template <typename... T, size_t... I> std::tuple<T *...> tryGetComponentsTuple(std::index_sequence<I...>)
    {
        return std::make_tuple((hasComponent<T>() ? &m_Scene->getRegistry().get<T>(m_EntityHandle) : nullptr)...);
    }
