...
template <typename... Component> class EntityView;

template <typename... Component> class EntityViewIterator;

// Entity wrapper that holds component references from the view
template <typename... Component> class ViewEntity {
  public:
    ...
    Entity getEntity() const { return Entity(m_EntityHandle, m_Scene); }
    ...
    template <typename... T> std::tuple<T &...> getComponents()
    {
    ...
    std::tuple<Component &...> getAllComponents() { return m_Components; }
    ...
    bool operator==(const ViewEntity &other) const { return m_EntityHandle == other.m_EntityHandle; }

    bool operator!=(const ViewEntity &other) const { return m_EntityHandle != other.m_EntityHandle; }

    bool operator==(const Entity &other) const { return m_EntityHandle == other.getHandle(); }
...
template <typename... Component> class EntityViewIterator {
  public:
    ...
    EntityViewIterator(typename entt::view<Component...>::iterator it, Scene *scene) : m_Iterator(it), m_Scene(scene) {}
    ...
    bool operator==(const EntityViewIterator &other) const { return m_Iterator == other.m_Iterator; }
...
template <typename... Component> class EntityView {
  public:
    ...
    const_iterator begin() const { return const_iterator(m_View.begin(), m_Scene); }

    const_iterator end() const { return const_iterator(m_View.end(), m_Scene); }
    ...
    bool contains(Entity entity) const { return m_View.contains(entity.getHandle()); }
    ...
    template <typename Func> void forEachRaw(Func &&func)
    {
        m_View.each([&func](auto entity, Component &...components) { func(entity, components...); });
    }

    // Get the underlying entt view (for advanced usage)
    const entt::view<Component...> &getRawView() const { return m_View; }
...
namespace Views {
template <typename... Component> EntityView<Component...> createView(Scene *scene)
{
...
template <typename... Component> class ScopedEntityView {
  public:
    ...
    template <typename Func> void forEach(Func &&func) { m_View.forEach(std::forward<Func>(func)); }
