...
class AssetManagerEditor : public AssetManagerBase {
  public:
    ...
    std::pair<std::shared_ptr<Asset>, AssetHandle> importAsset(std::filesystem::path path,
...
// - if a reference to the asset is stored -> less to no overhead, but, hard to serialize and link to the assets metadata, like
// filepath

// SOLUTION(s)? :
// - store the asset handle in the component with the underlying asset pointer -> (only possible if the asset handle cant be
// changed, so we can maybe store an asset pointer??)
