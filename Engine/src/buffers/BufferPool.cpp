...
#include "command_buffers/CommandBuffer.h"
#include "command_buffers/CommandPool.h"
...
{
    if (!isValid() || !data) {
        RP_CORE_ERROR("Invalid allocation or null data");
        return;
    }

    if (offset + size > sizeBytes) {
        RP_CORE_ERROR("Upload size {} + offset {} exceeds allocation size {}", size, offset, sizeBytes);
        return;
    }
...
BufferArena::BufferArena(uint32_t id, VmaAllocator allocator, VkDeviceSize arenaSize, BufferUsage usage,
    ...
    : id(id), vmaAllocator(allocator), size(arenaSize), usage(usage), usageFlags(usageFlags), flags(flags)
{
    ...
    if (result != VK_SUCCESS) {
        RP_CORE_ERROR("Failed to create virtual block with size {} MB", arenaSize / MEGA_BYTE);
        return;
    }
    ...
    if (result != VK_SUCCESS) {
        RP_CORE_ERROR("Failed to create buffer with size {} MB", arenaSize / MEGA_BYTE);
        vmaDestroyVirtualBlock(virtualBlock);
        virtualBlock = VK_NULL_HANDLE;
        return;
    }

    RP_CORE_INFO("Created arena {} with size {} MB", id, arenaSize / MEGA_BYTE);
}
...
void BufferArena::clear()
{
    ...
    RP_CORE_INFO("Destroyed arena {}", id);
}
...
bool BufferArena::isCompatible(const BufferAllocationRequest &request) const
{
    ...
    if (!(usageFlags & requestFlags)) {
        RP_CORE_ERROR("Arena {} does not have the required buffer usage flags", id);
        return false;
    }
    ...
    if (request.flags.useAccelerationStructure &&
...
BufferPoolManager::BufferPoolManager(VmaAllocator allocator)
    : m_allocator(allocator)
{
    RP_CORE_INFO("Initialized with VMA allocator");
}

BufferPoolManager::~BufferPoolManager()
{
    std::lock_guard<std::mutex> lock(m_mutex);
    m_layoutToArenaMap.clear();
    m_allocator = VK_NULL_HANDLE;
    RP_CORE_INFO("Shutdown complete");
}
...
std::shared_ptr<BufferAllocation> BufferPoolManager::allocateBuffer(const BufferAllocationRequest &request)
{
    ...
    if (!arena) {
        RP_CORE_ERROR("Failed to find or create arena for buffer allocation");
        return nullptr;
    }

    auto allocation = std::make_shared<BufferAllocation>();
    if (!arena->allocate(request.size, request.alignment, allocation.get())) {
        RP_CORE_ERROR("Failed to allocate {} bytes from arena {}", request.size, arena->id);
        return nullptr;
    }
...
std::shared_ptr<BufferArena> BufferPoolManager::createArena(const BufferAllocationRequest &request)
{
    ...
    if (arena->buffer == VK_NULL_HANDLE) {
        RP_CORE_ERROR("Failed to create arena {}", arenaId);
        return nullptr;
    }
