...
void reflectShaderInfo(const std::vector<char>& spirvCode) {
    ...
    if (result == SPV_REFLECT_RESULT_SUCCESS && count > 0) {
        ...
        for (uint32_t i = 0; i < count; ++i) {
            ...
            RP_CORE_INFO("    Binding {0}: set={1}, type={2}, name={3}", 
                binding.binding, binding.set, 
                descriptorTypeToString(binding.descriptor_type), 
    ...
    if (result == SPV_REFLECT_RESULT_SUCCESS && count > 0) {
        ...
        for (uint32_t i = 0; i < count; ++i) {
            auto& input = *inputs[i];
            RP_CORE_INFO("    Location {0}: name={1}, format={2}", 
                input.location, input.name ? input.name : "unnamed", "FORMAT");
        }
    }
    ...
    if (result == SPV_REFLECT_RESULT_SUCCESS && count > 0) {
        ...
        for (uint32_t i = 0; i < count; ++i) {
            ...
            RP_CORE_INFO("    Location {0}: name={1}, format={2}", 
                output.location, output.name ? output.name : "unnamed", "FORMAT");
        }
    }
    ...
    if (result == SPV_REFLECT_RESULT_SUCCESS && count > 0) {
        ...
        for (uint32_t i = 0; i < count; ++i) {
            ...
            RP_CORE_INFO("    Size: {0} bytes, name={1}", 
                pushConstant.size, pushConstant.name ? pushConstant.name : "unnamed");
        }
    }
        ...
        case SpvOpTypeVector:
            {
                ...
                if (typeDescription->type_flags & SPV_REFLECT_TYPE_FLAG_FLOAT) componentType = "float";
                    ...
                    // If UBO members are unsigned, they should ideally be uint in GLSL.
                     if (typeDescription->members && typeDescription->members[0].type_flags & SPV_REFLECT_TYPE_FLAG_INT && typeDescription->members[0].traits.numeric.scalar.signedness == 0) {
                        componentType = "uint";
                     } else {
                        componentType = "int";
                     }
        ...
        case SpvOpTypeMatrix:
            {
                ...
                if (typeDescription->traits.numeric.matrix.column_count == typeDescription->traits.numeric.matrix.row_count) {
                    ...
                    return prefix + std::to_string(typeDescription->traits.numeric.matrix.column_count) + "x" + std::to_string(typeDescription->traits.numeric.matrix.row_count);
                }
            }
