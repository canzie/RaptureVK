    ...
    : m_layout(VK_NULL_HANDLE), m_set(VK_NULL_HANDLE), m_setNumber(bindings.setNumber)
{
    ...
    if (s_poolRefCount > s_maxSets) {
        RP_CORE_ERROR("Too many descriptor sets! Current: {}, Max: {}", s_poolRefCount, s_maxSets);
    ...
    try {
        ...
        RP_CORE_ERROR("Failed to allocate descriptors: {}", e.what());
...
{
    vkCmdBindDescriptorSets(commandBuffer, pipeline->getPipelineBindPoint(), pipeline->getPipelineLayoutVk(), m_setNumber, 1,
                            &m_set, 0, nullptr);
}
...
void DescriptorSet::createBinding(const DescriptorSetBinding &binding)
{
    ...
    case VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER:
        m_uniformBufferBindings[binding.location] =
            std::make_shared<DescriptorBindingUniformBuffer>(this, bindNumber, binding.count);
        break;
    case VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER:
    case VK_DESCRIPTOR_TYPE_STORAGE_IMAGE:
        m_textureBindings[binding.location] = std::make_shared<DescriptorBindingTexture>(
            this, bindNumber, binding.viewType, binding.useStorageImageInfo, binding.count);
        break;
    ...
    case VK_DESCRIPTOR_TYPE_STORAGE_BUFFER:
        ...
        break;
    default:
        RP_CORE_ERROR("Unknown descriptor type: {}", static_cast<int>(binding.type));
        return;
    }
}
...
void DescriptorSet::createDescriptorSet()
{
    ...
    if (result != VK_SUCCESS) {
        RP_CORE_ERROR("Failed to allocate descriptor set for set {}! VkResult: {} ({})", m_setNumber, static_cast<int>(result),
                      result == VK_ERROR_OUT_OF_POOL_MEMORY ? "VK_ERROR_OUT_OF_POOL_MEMORY"
                      : result == VK_ERROR_FRAGMENTED_POOL  ? "VK_ERROR_FRAGMENTED_POOL"
                                                            : "OTHER");
        RP_CORE_ERROR("Pool status - Sets: {}/{}, Buffers: {}/{}, Textures: {}/{}, "
                      "StorageBuffers: {}/{}, StorageImages: {}/{}, AccelStructs: {}/{}",
                      s_poolRefCount, s_maxSets, s_poolBufferCount, s_maxBuffers, s_poolTextureCount, s_maxTextures,
...
void DescriptorSet::updateUsedCounts(const DescriptorSetBindings &bindings)
{
    ...
    RP_CORE_TRACE("DescriptorSet: Resource usage for set {} - Buffers: {}, Textures: {}, "
                  ...
                  bindings.setNumber, newBuffers, newTextures, newStorageBuffers, newStorageImages, newInputAttachments,
                  newAccelerationStructures);
    ...
    if (s_poolBufferCount + newBuffers > s_maxBuffers) {
        RP_CORE_ERROR("DescriptorSet: Uniform buffer limit exceeded for set {}! "
                      ...
                      bindings.setNumber, s_poolBufferCount, newBuffers, s_poolBufferCount + newBuffers, s_maxBuffers);
        throw std::runtime_error("DescriptorSet: Uniform buffer limit exceeded! Current: " + std::to_string(s_poolBufferCount) +
                                 ", Requested: " + std::to_string(newBuffers) + ", Max: " + std::to_string(s_maxBuffers));
    }

    if (s_poolTextureCount + newTextures > s_maxTextures) {
        RP_CORE_ERROR("DescriptorSet: Texture/Sampler limit exceeded for set {}! "
                      "Current: {}, Requested: {}, Total would be: {}, Max: {}",
                      bindings.setNumber, s_poolTextureCount, newTextures, s_poolTextureCount + newTextures, s_maxTextures);
        throw std::runtime_error("DescriptorSet: Texture/Sampler limit exceeded! Current: " + std::to_string(s_poolTextureCount) +
                                 ", Requested: " + std::to_string(newTextures) + ", Max: " + std::to_string(s_maxTextures));
    }
    ...
    if (s_poolStorageBufferCount + newStorageBuffers > s_maxStorageBuffers) {
        RP_CORE_ERROR("DescriptorSet: Storage buffer limit exceeded for set {}! "
                      "Current: {}, Requested: {}, Total would be: {}, Max: {}",
                      bindings.setNumber, s_poolStorageBufferCount, newStorageBuffers, s_poolStorageBufferCount + newStorageBuffers,
                      s_maxStorageBuffers);
        throw std::runtime_error(
            "DescriptorSet: Storage buffer limit exceeded! Current: " + std::to_string(s_poolStorageBufferCount) +
            ", Requested: " + std::to_string(newStorageBuffers) + ", Max: " + std::to_string(s_maxStorageBuffers));
    }
    ...
    if (s_poolStorageImageCount + newStorageImages > s_maxStorageImages) {
        RP_CORE_ERROR("DescriptorSet: Storage image limit exceeded for set {}! "
                      "Current: {}, Requested: {}, Total would be: {}, Max: {}",
                      bindings.setNumber, s_poolStorageImageCount, newStorageImages, s_poolStorageImageCount + newStorageImages,
                      s_maxStorageImages);
        throw std::runtime_error(
            "DescriptorSet: Storage image limit exceeded! Current: " + std::to_string(s_poolStorageImageCount) +
            ", Requested: " + std::to_string(newStorageImages) + ", Max: " + std::to_string(s_maxStorageImages));
    }
    ...
    if (s_poolInputAttachmentCount + newInputAttachments > s_maxInputAttachments) {
        ...
        throw std::runtime_error(
            "DescriptorSet: Input attachment limit exceeded! Current: " + std::to_string(s_poolInputAttachmentCount) +
            ", Requested: " + std::to_string(newInputAttachments) + ", Max: " + std::to_string(s_maxInputAttachments));
    }
    ...
    if (s_poolAccelerationStructureCount + newAccelerationStructures > s_maxAccelerationStructures) {
        ...
        throw std::runtime_error(
            "DescriptorSet: Acceleration structure limit exceeded! Current: " + std::to_string(s_poolAccelerationStructureCount) +
            ", Requested: " + std::to_string(newAccelerationStructures) + ", Max: " + std::to_string(s_maxAccelerationStructures));
    }
