...
struct RigidBodyComponent {
    ...
    RigidBodyComponent(std::unique_ptr<Entropy::ColliderBase> collider) : collider(std::move(collider)) {}
    ...
    void setInertiaTensor(const glm::mat3 &inertiaTensor) { invInertiaTensor = glm::inverse(inertiaTensor); }
};
