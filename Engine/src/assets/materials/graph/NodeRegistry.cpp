...
static void s_validateDefinition(const NodeDefinition &def)
{
    auto checkPins = [&](const std::vector<PinDef> &pins, const char *direction) {
        ...
        for (const auto &pin : pins) {
            ...
            RP_ASSERT(unique, "node type '{}' has a duplicate {} pin name '{}'", Graph_nodeTypeName(def.type), direction, pin.name);
        }
    };
...
void NodeRegistry::registerBuiltins()
{
    ...
    registerNode(
        {.type = GraphNodeType::TERRAIN_HEIGHT, .outputs = {{"out", PinType::FLOAT}}, .glslTemplate = "{$normalizedHeight}"});
    registerNode({.type = GraphNodeType::TERRAIN_CURVATURE, .outputs = {{"out", PinType::FLOAT}}, .glslTemplate = "{$curvature}"});
    registerNode({.type = GraphNodeType::TERRAIN_LOD, .outputs = {{"out", PinType::INT}}, .glslTemplate = "{$chunkLod}"});
    registerNode({.type = GraphNodeType::TERRAIN_EROSION, .outputs = {{"out", PinType::FLOAT}}, .glslTemplate = "{$erosion}"});
    registerNode({.type = GraphNodeType::TERRAIN_CONTINENTALNESS,
                  .outputs = {{"out", PinType::FLOAT}},
                  .glslTemplate = "{$continentalness}"});
    registerNode(
        {.type = GraphNodeType::TERRAIN_PEAKS_VALLEYS, .outputs = {{"out", PinType::FLOAT}}, .glslTemplate = "{$peaksValleys}"});
