...
TLAS::TLAS()
    : m_accelerationStructure(VK_NULL_HANDLE), m_buffer(VK_NULL_HANDLE), m_allocation(VK_NULL_HANDLE),
      m_instanceBuffer(VK_NULL_HANDLE), m_instanceAllocation(VK_NULL_HANDLE), m_scratchBuffer(VK_NULL_HANDLE),
      m_scratchAllocation(VK_NULL_HANDLE), m_deviceAddress(0), m_accelerationStructureSize(0), m_scratchSize(0), m_isBuilt(false),
      m_needsRebuild(false), m_supportsUpdate(true) // Assume true for now, could check device features
      ,
...
void TLAS::createInstanceBuffer()
{
    ...
    // Fill instance data
    VkAccelerationStructureInstanceKHR *instanceData = static_cast<VkAccelerationStructureInstanceKHR *>(allocInfo.pMappedData);
...
void TLAS::updateInstances(const std::vector<std::pair<uint32_t, glm::mat4>> &instanceUpdates)
{
    ...
    if (m_supportsUpdate) {
        ...
        if (vmaCreateBuffer(m_allocator, &scratchBufferCreateInfo, &scratchAllocCreateInfo, &updateScratchBuffer,
                            &updateScratchAllocation, nullptr) != VK_SUCCESS) {
...
void TLAS::updateInstanceBuffer(const std::vector<std::pair<uint32_t, glm::mat4>> &instanceUpdates)
{
    ...
    for (const auto &[instanceIndex, newTransform] : instanceUpdates) {
        if (instanceIndex < m_instances.size()) {
            ...
            instanceData[instanceIndex].instanceShaderBindingTableRecordOffset =
                m_instances[instanceIndex].shaderBindingTableRecordOffset;
