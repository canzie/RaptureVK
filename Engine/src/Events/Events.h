...
template <typename... Args> class EventHandler : public BaseEventHandler {
  public:
    EventHandler() = default;

    EventBus<Args...> &getEventBus() { return m_eventBus; }

    void clearAllListeners() override { m_eventBus.clearListeners(); }
...
class EventRegistry {
  public:
    ...
    template <typename... Args> EventBus<Args...> &getEventBus(const std::string &name)
    {
        ...
        if (busIt == m_eventBuses.end() || m_eventTypeNames[name] != typeId) {
