...
std::vector<std::shared_ptr<UniformBuffer>>
    ForwardRenderer::m_cameraUniformBuffers = {};
...
bool ForwardRenderer::m_framebufferNeedsResize = false;
...
void ForwardRenderer::init() {
  ...
  InputEvents::onMouseScrolled().addListener(
      [](float x, float y) { m_zoom += y; });
  ...
  ApplicationEvents::onSwapChainRecreated().addListener(
      [](std::shared_ptr<SwapChain> swapChain) { recreateSwapChain(); });
}
...
void ForwardRenderer::setupShaders() {
  ...
  const std::string vertShaderPath = "SPIRV/pbr.vs.spv";

  auto [shader, handle] =
      AssetManager::importAsset<Shader>(shaderPath / vertShaderPath);
  m_shader = shader;
}
...
FramebufferSpecification ForwardRenderer::getMainFramebufferSpecification() {
  if (m_swapChain->getImageFormat() == VK_FORMAT_UNDEFINED) {
    RP_CORE_ERROR("ForwardRenderer - Attempted to create render pass before "
                  "swap chain was initialized!");
    throw std::runtime_error("ForwardRenderer - Attempted to create render "
                             "pass before swap chain was initialized!");
  }
...
void ForwardRenderer::setupDynamicRenderingMemoryBarriers(
  ...
  colorBarrier.oldLayout =
      VK_IMAGE_LAYOUT_UNDEFINED; // Always start from undefined for the first
                                 // transition
  ...
  if (m_swapChain->getDepthTexture() &&
      m_swapChain->getDepthTexture()->getImage()) {
    depthBarrier.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
    depthBarrier.oldLayout =
        VK_IMAGE_LAYOUT_UNDEFINED; // Always start from undefined for depth
    depthBarrier.newLayout = VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;
    depthBarrier.srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
    depthBarrier.dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
    depthBarrier.image = m_swapChain->getDepthTexture()->getImage();
    depthBarrier.subresourceRange.aspectMask = VK_IMAGE_ASPECT_DEPTH_BIT;
    if (m_swapChain->getDepthImageFormat() == VK_FORMAT_D32_SFLOAT_S8_UINT ||
        m_swapChain->getDepthImageFormat() == VK_FORMAT_D24_UNORM_S8_UINT) {
...
void ForwardRenderer::beginDynamicRendering(
    std::shared_ptr<CommandBuffer> commandBuffer) {
  ...
  bool hasDepth =
      m_swapChain->getDepthTexture() &&
      m_swapChain->getDepthTexture()->getImageView() != VK_NULL_HANDLE;
  if (hasDepth) {
    depthAttachmentInfo.sType = VK_STRUCTURE_TYPE_RENDERING_ATTACHMENT_INFO;
    depthAttachmentInfo.imageView =
        m_swapChain->getDepthTexture()->getImageView();
    depthAttachmentInfo.imageLayout =
        VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;
    depthAttachmentInfo.loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
    depthAttachmentInfo.storeOp =
        VK_ATTACHMENT_STORE_OP_DONT_CARE; // Or STORE if needed later
  ...
  renderingInfo.pDepthAttachment =
      hasDepth ? &depthAttachmentInfo : VK_NULL_HANDLE;
...
void ForwardRenderer::recordCommandBuffer(
    std::shared_ptr<CommandBuffer> commandBuffer, uint32_t imageIndex,
  ...
  auto view =
      registry.view<TransformComponent, MeshComponent, MaterialComponent>();
  ...
  for (auto entity : view) {
    ...
    if (vulkanContext.isVertexInputDynamicStateEnabled() &&
        vulkanContext.vkCmdSetVertexInputEXT) {
      ...
      vulkanContext.vkCmdSetVertexInputEXT(
          commandBuffer->getCommandBufferVk(), 1, &bindingDescription,
    ...
    vkCmdBindIndexBuffer(commandBuffer->getCommandBufferVk(),
                         mesh->getIndexBuffer()->getBufferVk(), 0,
                         mesh->getIndexBuffer()->getIndexType());

    // Draw the mesh
    vkCmdDrawIndexed(commandBuffer->getCommandBufferVk(), mesh->getIndexCount(),
                     1, 0, 0, 0);
  }
  ...
  if (activeScene) {
    ...
    for (auto entity : cameraView) {
      ...
      if (camera.isMainCamera) {
        ...
        float aspectRatio = m_swapChain->getExtent().width /
                            (float)m_swapChain->getExtent().height;
        if (camera.aspectRatio != aspectRatio) {
          camera.updateProjectionMatrix(camera.fov, aspectRatio,
...
void ForwardRenderer::createDescriptorPool() {
  ...
  poolSizes[0].descriptorCount =
      static_cast<uint32_t>(m_swapChain->getImageCount() * 2);
...
void ForwardRenderer::updateLights(std::shared_ptr<Scene> activeScene) {
  ...
  auto &registry = activeScene->getRegistry();
  auto lightView =
      registry.view<TransformComponent, LightComponent, TagComponent>();
