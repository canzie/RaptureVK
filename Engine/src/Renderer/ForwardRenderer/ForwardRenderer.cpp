...
void ForwardRenderer::setupShaders()
{
    ...
    auto [shader, handle] = AssetManager::importAsset<Shader>(shaderPath / vertShaderPath);
    m_shader = shader;
}
...
{
    auto &app = Application::getInstance();
    (void)app.getVulkanContext();

    VkImageMemoryBarrier colorBarrier{};
    // Image layout transitions for dynamic rendering
    colorBarrier.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
    colorBarrier.oldLayout = VK_IMAGE_LAYOUT_UNDEFINED; // Always start from undefined for the first
                                                        // transition
...
void ForwardRenderer::recordCommandBuffer(std::shared_ptr<CommandBuffer> commandBuffer, uint32_t imageIndex,
                                          std::shared_ptr<Scene> activeScene)
{
    ...
    for (auto entity : view) {
        ...
        (void)view.get<MaterialComponent>(entity);
        ...
        if (vulkanContext.isVertexInputDynamicStateEnabled() && vulkanContext.vkCmdSetVertexInputEXT) {
            ...
            vulkanContext.vkCmdSetVertexInputEXT(commandBuffer->getCommandBufferVk(), 1, &bindingDescription,
                                                 static_cast<uint32_t>(attributeDescriptions.size()), attributeDescriptions.data());
        }

        // Push the model matrix as a push constant
        ForwardRendererPushConstants pushConstants{};
        pushConstants.model = transform.transformMatrix();
        pushConstants.camPos = transform.translation();

        vkCmdPushConstants(commandBuffer->getCommandBufferVk(), m_graphicsPipeline->getPipelineLayoutVk(),
                           VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(ForwardRendererPushConstants),
                           &pushConstants);
        ...
        // Bind index buffer
        vkCmdBindIndexBuffer(commandBuffer->getCommandBufferVk(), mesh->getIndexBuffer()->getBufferVk(), 0,
...
void ForwardRenderer::updateUniformBuffers()
{
    ...
    if (activeScene) {
        ...
        ubo.view = glm::lookAt(glm::vec3(2.0f, 2.0f, 2.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 0.0f, 1.0f));
        ubo.proj = glm::perspective(glm::radians(45.0f), m_swapChain->getExtent().width / (float)m_swapChain->getExtent().height,
                                    0.1f, 10.0f);
    }
