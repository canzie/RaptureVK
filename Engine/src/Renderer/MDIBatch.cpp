...
void MDIBatch::uploadBuffers()
{
    ...
    if (!m_buffersCreated || requiredSize > m_allocatedSize) {
        ...
        m_indirectBuffer = std::make_shared<StorageBuffer>(newSize * sizeof(VkDrawIndexedIndirectCommand), BufferUsage::STREAM,
                                                           allocator, VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT);
