...
GraphEditor::GraphEditor(const char *label, std::shared_ptr<Graph> graph, ImVec2 size)
    : m_label(label), m_graph(graph), m_size(size), m_scrolling(ImVec2(0.0f, 0.0f)), m_zoom(1.0f),
      m_isDraggingConnection(false), m_isOutputPin(false), m_connectionSourceNode(0),
      m_connectionSourceParam(0), m_connectionDragPos(ImVec2(0.0f, 0.0f))
{
}
...
void GraphEditor::renderGrid()
{
    ...
    for (float x = offsetX - minorGridSize; x < panelSize.x; x += minorGridSize) {
        drawList->AddLine(ImVec2(panelPos.x + x, panelPos.y),
                         ImVec2(panelPos.x + x, panelPos.y + panelSize.y),
                         IM_COL32(80, 80, 80, 40), 1.0f);
    }
    for (float y = offsetY - minorGridSize; y < panelSize.y; y += minorGridSize) {
        drawList->AddLine(ImVec2(panelPos.x, panelPos.y + y),
                         ImVec2(panelPos.x + panelSize.x, panelPos.y + y),
                         IM_COL32(80, 80, 80, 40), 1.0f);
    }
...
void GraphEditor::renderNode(GraphNode &node)
{
    ...
    drawList->AddRectFilled(screenPos, ImVec2(screenPos.x + screenSize.x, screenPos.y + headerHeight * m_zoom),
                           ImGui::ColorConvertFloat4ToU32(ImVec4(node.color.x * 0.7f, node.color.y * 0.7f, node.color.z * 0.7f, 1.0f)),
    ...
    drawList->AddText(ImGui::GetFont(), ImGui::GetFontSize() * textScale, ImVec2(titleX, titleY), IM_COL32_WHITE, node.name.c_str());
...
void GraphEditor::renderInputPins(GraphNode &node, ImVec2 screenPos, float headerHeight, float lineHeight, float textScale)
{
    ...
    for (size_t i = 0; i < node.inputs.size(); ++i) {
        ...
        if (hasConnection) {
            drawList->AddCircleFilled(pinScreenPos, pinRadius, ImGui::ColorConvertFloat4ToU32(node.inputs[i].color));
        } else {
            drawList->AddCircle(pinScreenPos, pinRadius, ImGui::ColorConvertFloat4ToU32(node.inputs[i].color), 12, 2.0f);
        }

        ImVec2 labelPos = ImVec2(pinScreenPos.x + pinRadius + 4.0f * m_zoom, screenPos.y + currentY * m_zoom);
        drawList->AddText(ImGui::GetFont(), ImGui::GetFontSize() * textScale, labelPos, IM_COL32(220, 220, 220, 255), node.inputs[i].name.c_str());
        ...
        if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenBlockedByActiveItem) && ImGui::IsMouseReleased(ImGuiMouseButton_Left) && m_isDraggingConnection && m_isOutputPin) {
...
void GraphEditor::renderOutputPins(GraphNode &node, ImVec2 screenPos, float headerHeight, float lineHeight, float textScale)
{
    ...
    for (size_t i = 0; i < node.outputs.size(); ++i) {
        ImVec2 pinCanvasPos = ImVec2(node.windowPosition.x + node.windowSize.x, node.windowPosition.y + currentY + lineHeight * 0.5f);
        ImVec2 pinScreenPos = canvasToScreen(pinCanvasPos);
        float pinRadius = 6.0f * m_zoom;
        ...
        if (hasConnection) {
            drawList->AddCircleFilled(pinScreenPos, pinRadius, ImGui::ColorConvertFloat4ToU32(node.outputs[i].color));
        } else {
            drawList->AddCircle(pinScreenPos, pinRadius, ImGui::ColorConvertFloat4ToU32(node.outputs[i].color), 12, 2.0f);
        }

        ImVec2 textSize = ImGui::CalcTextSize(node.outputs[i].name.c_str());
        ImVec2 labelPos = ImVec2(pinScreenPos.x - pinRadius - 4.0f * m_zoom - textSize.x * textScale, screenPos.y + currentY * m_zoom);
        drawList->AddText(ImGui::GetFont(), ImGui::GetFontSize() * textScale, labelPos, IM_COL32(220, 220, 220, 255), node.outputs[i].name.c_str());
...
void GraphEditor::handleNodeInteraction(GraphNode &node, ImVec2 screenPos, ImVec2 screenSize, float headerHeight)
{
    ...
    for (int i = 1; i <= 3; ++i) {
        ...
        drawList->AddLine(ImVec2(screenPos.x + screenSize.x - offset, screenPos.y + screenSize.y),
                         ImVec2(screenPos.x + screenSize.x, screenPos.y + screenSize.y - offset),
