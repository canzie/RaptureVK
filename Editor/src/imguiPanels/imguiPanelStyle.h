...
// Named style mapping
inline const std::unordered_map<std::string, ImVec4> NamedColors = {{"background_primary", BACKGROUND_PRIMARY},
...
inline void InitializeFonts()
{
    ...
    s_RegularFont = io.Fonts->AddFontFromFileTTF("assets/fonts/Roboto-Regular.ttf", 16.0f);
    s_BoldFont = io.Fonts->AddFontFromFileTTF("assets/fonts/Roboto-Bold.ttf", 16.0f);
    s_LightFont = io.Fonts->AddFontFromFileTTF("assets/fonts/Roboto-Light.ttf", 16.0f);
    s_ItalicFont = io.Fonts->AddFontFromFileTTF("assets/fonts/Roboto-Italic.ttf", 16.0f);
